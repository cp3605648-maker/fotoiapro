...
import { getCategoryProfileByPresetKey, buildCategoryProfileBlock } from "./categoryProfiles";
import { detectSubcategoryBlock } from "./subcategoryProfiles";
import { detectSocialPlatformBlock } from "./socialProfiles";
import { buildFreeformDetailsBlock } from "./freeformDetails";
import { buildPrioritySystemBlock } from "./prioritySystem";
import { buildVisualRiskBlock } from "./visualRiskEngine";
import { buildRequestAnalysisBlock } from "./requestAnalyzer";
import { REFERENCE_IDENTITY_RULES } from "../referenceIdentityRules";
import { buildReferenceInteraction } from "../referenceInteraction";
...
export function buildPrompt(userPrompt, isPaid, imageMeta = null) {
  const selectedPresetData = detectPreset(userPrompt);
  const selectedPreset = selectedPresetData?.preset || null;
  const selectedPresetKey = selectedPresetData?.key || null;
  const categoryProfile = getCategoryProfileByPresetKey(selectedPresetKey);
  const categoryProfileBlock = buildCategoryProfileBlock(categoryProfile);
  const subcategoryBlock = detectSubcategoryBlock(selectedPresetKey, userPrompt);
  const socialPlatformBlock = detectSocialPlatformBlock(userPrompt);
  const freeformDetailsBlock = buildFreeformDetailsBlock(userPrompt);
  const prioritySystemBlock = buildPrioritySystemBlock(userPrompt);
  const visualRiskBlock = buildVisualRiskBlock(userPrompt, imageMeta);
  const requestAnalysisBlock = buildRequestAnalysisBlock(userPrompt);
  const referenceInteractionBlock = buildReferenceInteraction(userPrompt);
...
${categoryProfileBlock}

${subcategoryBlock}

${socialPlatformBlock}

${freeformDetailsBlock}

${prioritySystemBlock}

${visualRiskBlock}

${REFERENCE_IDENTITY_RULES}

${referenceInteractionBlock}
...
identity swap, face swap, mixed face, hybrid face, blended identities,
reference face transferred to main subject, main subject face transferred to reference,
older face, younger face, beautified face, plastic skin, overly smooth skin,
different eye shape, different nose shape, different mouth shape, different jawline,
distorted face, bad eyes, bad teeth,
